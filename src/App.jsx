...
export default function App(){
  ...
  // Sync done-state to EXACTLY match the backend (Render = single source of truth).
  // This both marks teammates' done ASINs AND clears any stale local-only "done"
  // flags that were never saved to the server, so every screen shows the same
  // count and it can never drift above the real shared number.
  useEffect(()=>{
    let cancelled=false;
    apiFetchDone().then(done=>{
      if(cancelled||!Array.isArray(done))return;
      const doneSet=new Set(done.map(a=>String(a).trim()));
      setAsinSt(prev=>{
        const next={...prev};
        // mark server-done ASINs done
        doneSet.forEach(asin=>{
          const ex=next[asin]||{decisions:{},comments:{},verified:{},done:false,notes:"",by:""};
          if(!ex.done)next[asin]={...ex,done:true};
        });
        // clear local "done" flags that the server does NOT have (stale/local-only)
        Object.keys(next).forEach(asin=>{
          if(next[asin]?.done && !doneSet.has(asin)){
            next[asin]={...next[asin],done:false};
          }
        });
        return next;
      });
